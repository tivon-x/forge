"""Best-effort PyPI update checks for the Forge CLI."""
...
UPDATE_CHECK_ENV_DISABLE = "FORGE_NO_UPDATE_CHECK"
UPDATE_CHECK_ENV_ENABLE = "FORGE_ENABLE_UPDATE_CHECK"
...
class UpdateNotice:
    ...
    def message(self) -> str:
        ...
        return (
            f"Forge {self.latest_version} is available (installed: {self.current_version}). "
...
class ReleaseNotesEntry:
    """Structured release notes for one Forge version."""
...
class ReleaseNotesNotice:
    """Release notes shown once after the installed Forge version changes."""
    ...
    def message(self) -> str:
        ...
        return f"Forge updated to {self.current_version}\n\n{body}"
...
def startup_update_notice(
    ...
    *,
    ...
    """Return an update notice when PyPI has a newer stable Forge release.
    ...
    if _update_check_disabled(environment):
        return None
    if fetcher is None and environment.get(UPDATE_CHECK_ENV_ENABLE, "").strip().lower() not in {
        "1",
        "true",
        "yes",
        "on",
...
def startup_release_notes_notice(
    ...
    *,
    ...
    """Return release notes once after Forge starts with a newer installed version.
...
def fetch_latest_pypi_version(*, fetcher: Fetcher | None = None) -> str | None:
    """Fetch the latest stable Forge version from PyPI."""
...
def default_update_check_cache_path(paths: ForgePaths | None = None) -> Path:
    """Return the on-disk cache path for startup update checks."""
    return (paths or ForgePaths()).home / "cache" / "update-check.json"


def default_release_notes_state_path(paths: ForgePaths | None = None) -> Path:
    """Return the on-disk state path for one-time release notes."""
    return (paths or ForgePaths()).home / "cache" / RELEASE_NOTES_STATE_FILENAME
