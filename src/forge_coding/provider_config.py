"""Durable provider configuration for Forge coding sessions."""
...
class ProviderConfigError(ValueError):
    """Raised when Forge provider configuration is invalid."""
...
class ProviderSettings:
    """Forge provider settings loaded from Forge home."""
...
class ProviderSelection:
    """Resolved provider/model selection for a Forge run."""
...
def builtin_provider_configs() -> tuple[ProviderConfig, ...]:
    """Return Forge's built-in provider configs."""
    return tuple(
...
def default_openai_provider_config() -> OpenAICompatibleProviderConfig:
    """Return Forge's default OpenAI-compatible provider entry."""
...
def provider_settings_path(paths: ForgePaths | None = None) -> Path:
    """Return the durable provider settings path."""
    return (paths or ForgePaths()).home / "providers.json"
...
def load_provider_settings(paths: ForgePaths | None = None) -> ProviderSettings:
    ...
    resolved_paths = paths or ForgePaths()
...
def save_provider_settings(settings: ProviderSettings, paths: ForgePaths | None = None) -> Path:
...
def _effective_provider_configs(paths: ForgePaths | None = None) -> tuple[ProviderConfig, ...]:
    ...
    *,
    ...
    if provider.thinking_levels is None:
        ...
        if isinstance(provider, OpenAICodexProviderConfig):
            return (
                "OpenAI Codex subscription can stream reasoning output, but Forge does "
...
def _thinking_level_map_supports(
    ...
    # Pi semantics: levels absent from a model's map use the provider default
    # through "high"; the extended "xhigh" and "max" levels require an explicit
    # entry.
...
def provider_has_usable_credentials(
    ...
    *,
    ...
    """Return whether Forge can attempt calls for this provider without prompting setup."""
...
def _reasoning_effort_from_provider(
    ...
    *,
    ...
    if provider.name == "huggingface" and normalized == "minimal":
        ...
        # for reasoning_effort, but rejects Pi/Forge's "minimal" label.
        return "low"
