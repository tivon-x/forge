"""Canonical filesystem paths for Forge user and project data."""
...
class ForgePaths:
    """Resolved Forge filesystem locations.

    Forge keeps durable application data under the user's home directory while also
    loading project-local resources from the active working directory.
    """

    home: Path = field(default_factory=lambda: Path.home() / ".forge")
    ...
    def logs_dir(self) -> Path:
        """Return Forge's user-level diagnostic log directory."""
    ...
    def user_skills_dir(self) -> Path:
        """Return Forge's user-level skills directory."""
        return self.home / "skills"

    @property
    def user_prompts_dir(self) -> Path:
        """Return Forge's user-level prompt templates directory."""
    ...
    def project_forge_dir(self, cwd: Path) -> Path:
        """Return the project-local Forge resource directory."""
        return cwd / ".forge"
    ...
    def project_skills_dir(self, cwd: Path) -> Path:
        """Return the project-local Forge skills directory."""
        return self.project_forge_dir(cwd) / "skills"
