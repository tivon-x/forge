...
def _visible_footer_bindings(app: ForgeTuiApp) -> dict[str, str]:
...
def test_dark_theme_markdown_code_uses_aqua_highlight() -> None:
    console = Console(record=True, width=80)
    console.print(render_chat_item(ChatItem(role="assistant", text="Use `forge` here.")))
...
def test_dark_theme_markdown_bullets_use_theme_bullet_color() -> None:
    ...
    assert _style_color_escape(FORGE_DARK_THEME.markdown_bullet) in output
...
async def test_textual_markdown_widget_uses_theme_link_style() -> None:
    ...
    assert markdown.forge_link_style == FORGE_DARK_THEME.markdown_link


def test_textual_markdown_uses_theme_highlight_and_aqua_inline_code() -> None:
    variables = _theme_css_variables(FORGE_LIGHT_THEME)

    assert variables["forge-markdown-highlight"] == FORGE_LIGHT_THEME.markdown_heading
    assert variables["forge-markdown-table-header"] == FORGE_LIGHT_THEME.markdown_table_header
    assert variables["forge-markdown-table-border"] == FORGE_LIGHT_THEME.markdown_table_border
    assert variables["forge-markdown-inline-code"] == FORGE_LIGHT_THEME.markdown_inline_code
    assert (
        variables["forge-markdown-code-block-background"]
        == FORGE_LIGHT_THEME.markdown_code_block_background
    )
    assert variables["forge-markdown-link"] == FORGE_LIGHT_THEME.markdown_link
    assert variables["forge-markdown-bullet"] == FORGE_LIGHT_THEME.markdown_bullet
...
def test_light_theme_markdown_code_uses_aqua_without_background() -> None:
    ...
    console.print(
        render_chat_item(
            ChatItem(role="assistant", text="Use `forge` here."),
            theme=FORGE_LIGHT_THEME,
        )
    )
...
async def test_streaming_transcript_applies_role_foreground() -> None:
    ...
    async with app.run_test(size=(60, 20)) as pilot:
        ...
        thinking_fg, _ = _split_rich_style_colors(FORGE_DARK_THEME.role_styles["thinking"].body)
        assistant_fg, _ = _split_rich_style_colors(FORGE_DARK_THEME.role_styles["assistant"].body)
        ...
        await transcript.append_thinking_delta(
            "reasoning", theme=FORGE_DARK_THEME, show_thinking=True
        )
        await pilot.pause()
        thinking = next(
...
async def test_tool_transcript_uses_native_markdown_without_custom_selection_painting() -> None:
    app = ForgeTuiApp(FakeSession(messages=[]))
    item = ChatItem(
        role="tool",
        text="→ read README.md",
        tool_result_text="✓ read\nREADME contents",
...
async def test_forge_markdown_block_remains_selectable_after_mount() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        block = app.query_one(ForgeMarkdownBlock)
...
def test_tui_app_uses_configured_theme_css_variables() -> None:
    app = ForgeTuiApp(FakeSession(), tui_settings=TuiSettings(theme="high-contrast"))

    variables = app.get_theme_variable_defaults()

    assert variables["forge-screen-background"] == "#000000"
    assert variables["forge-prompt-background"] == "#1a1a1a"
    assert variables["forge-prompt-border"] == "#00ff66"
...
def test_tui_app_uses_light_theme_css_variables() -> None:
    app = ForgeTuiApp(FakeSession(), tui_settings=TuiSettings(theme="forge-light"))

    variables = app.get_theme_variable_defaults()

    assert variables["forge-screen-background"] == "#ffffff"
    assert variables["forge-chrome-background"] == "#f3f4f6"
    assert variables["forge-muted-text"] == "#475569"
    assert variables["forge-prompt-background"] == "#f8fafc"
    assert variables["forge-prompt-border"] == "#2563eb"
...
def test_tui_app_registers_only_forge_themes_with_textual() -> None:
    app = ForgeTuiApp(FakeSession())

    assert tuple(app.available_themes) == ("forge-dark", "forge-light", "high-contrast")


def test_textual_theme_mapping_uses_forge_theme_values() -> None:
    textual_theme = _textual_theme_for_forge_theme("forge-light")

    assert textual_theme.name == "forge-light"
    assert textual_theme.primary == FORGE_LIGHT_THEME.accent
    assert textual_theme.dark is False
    assert textual_theme.variables["forge-screen-background"] == FORGE_LIGHT_THEME.screen_background


def test_forge_dark_theme_uses_black_chat_backgrounds() -> None:
...
def test_forge_light_theme_uses_light_chat_backgrounds() -> None:
    theme = TuiSettings(theme="forge-light").resolved_theme
...
@pytest.mark.anyio
async def test_textual_theme_change_persists_forge_theme(
    ...
    async with app.run_test() as pilot:
        app.theme = "forge-light"
        await pilot.pause()

    assert app.tui_settings.theme == "forge-light"
    assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "forge-light"') != -1
...
async def test_tui_app_theme_command_opens_picker_and_persists_selection(
    ...
    async with app.run_test() as pilot:
        ...
        assert app.tui_settings.theme == "forge-light"
        assert app.theme == "forge-light"
        assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "forge-light"') != -1
        assert app.get_theme_variable_defaults()["forge-screen-background"] == "#ffffff"
...
async def test_tui_app_theme_command_argument_updates_theme_and_persists(
    ...
    async with app.run_test() as pilot:
        prompt = app.query_one("#prompt")
        prompt.value = "/theme forge-light"
        await pilot.press("enter")
...
async def test_tui_app_blocks_tree_picker_while_agent_is_running() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert notifications == [
            "Forge is still working. Press Escape to interrupt before using /tree."
        ]
...
async def test_tui_app_tree_picker_branches_with_summary() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert str(rendered_labels[0].spans[0].style) == _style_rgb(FORGE_DARK_THEME.accent)
...
async def test_tui_app_session_modal_auto_copies_selected_text(
    ...
    app = ForgeTuiApp(FakeSession(), tui_settings=TuiSettings(auto_copy_selection=False))
    copied: list[str] = []
    monkeypatch.setattr(app, "copy_to_clipboard", copied.append)
...
async def test_tui_login_saves_provider_key(
    ...
    assert (tmp_path / ".forge" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_openai_codex_saves_oauth_credentials(
    ...
    credentials = (tmp_path / ".forge" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_custom_provider_writes_catalog_and_preferences(
    ...
    assert FileCredentialStore(tmp_path / ".forge" / "credentials.json").get("nebius") == (
        "stored-nebius-key"
    )
    assert session.provider_reload_count == 1
...
async def test_tui_login_preserves_existing_scoped_models_and_providers(
    ...
    assert FileCredentialStore(forge_home / "credentials.json").get("openrouter") == (
...
async def test_tui_logout_removes_stored_api_key(
    ...
    isolate_home(monkeypatch, tmp_path)
    credential_path = tmp_path / ".forge" / "credentials.json"
...
async def test_tui_logout_opens_stored_credential_provider_picker(
    ...
    isolate_home(monkeypatch, tmp_path)
    FileCredentialStore(tmp_path / ".forge" / "credentials.json").set("anthropic", "stored-key")
    app = ForgeTuiApp(FakeSession())
...
async def test_tui_prompt_worker_refreshes_directly() -> None:
    app = ForgeTuiApp(FakeSession(events=[AgentStartEvent(), AgentEndEvent()]))
    refreshes = 0
...
async def test_tui_prompt_worker_shows_diagnostic_log_path_for_error_event(tmp_path: Path) -> None:
    class ErrorSession(FakeSession):
        def __init__(self) -> None:
            ...
            self.last_diagnostic_log_path = tmp_path / "forge-home" / "logs" / "agent-calls.jsonl"
...
async def test_tui_app_shows_startup_update_notice_in_transcript_only() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        transcript = app.query_one("#transcript", TranscriptView)
        assert [line.text for line in transcript.lines] == [
            "Forge updated to 0.2.0",
            "Forge 0.2.0 is available",
            "Earlier prompt",
        ]
...
async def test_tui_app_runs_initial_prompt() -> None:
    ...
    app = ForgeTuiApp(session, initial_prompt="explain this repo")
...
async def test_run_tui_app_opens_when_provider_login_is_missing(
    ...
    class FakeApp:
        def __init__(self, session: str, **kwargs: object) -> None:
            ...
            assert "Forge 0.2.0 is available" not in message
            assert kwargs["startup_notices"] == ("Forge 0.2.0 is available",)
