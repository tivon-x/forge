...
import forge_coding.update_check as update_check
...
def test_startup_update_notice_is_disabled_by_default(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        update_check,
        "fetch_latest_pypi_version",
        lambda **_kwargs: (_ for _ in ()).throw(AssertionError("no fetch")),
    )
...
def test_startup_release_notes_notice_reports_upgrade_once(tmp_path) -> None:
    ...
    assert notice.message == "Forge updated to 0.1.2\n\n**New**\n- New feature\n- Bug fix"
