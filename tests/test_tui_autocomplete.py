...
def test_theme_argument_completion_uses_theme_names() -> None:
    state = build_completion_state(
        ...
        skills=(),
        prompt_templates=(),
        theme_names=("forge-dark", "forge-light", "high-contrast"),
    )

    assert [item.display for item in state.items] == ["forge-dark", "forge-light"]
    assert state.selected is not None
    assert state.selected.apply("/theme forge-") == "/theme forge-dark"
